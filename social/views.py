...
def ticket(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = f"{cd['name']}\n{cd['email']}\n{cd['phone']}\n\n{cd['message']}"
            send_mail(cd['subject'], message, 'kingnima949@gmail.com', ['mohammadmahdi11121382@gmail.com'],
                      fail_silently=False)
...
def post_list(request, tag_slug=None):
    ...
    try:
        ...
        posts = []
    if request.headers.get('X-request-with') == 'XMLHttpResponse':
...
def create_post(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.save_m2m()
            return HttpResponse('پست با موفقیت ایجاد شد')
    else:
        form = CreatePostForm()
...
def like_post(request):
    ...
    if post_id is not None:
        ...
        response_data = {'liked': liked, 'likes_count': post_like_count}
    else:
...
def user_follow(request):
    ...
    if user_id:
        try:
            ...
            return JsonResponse(
                {'follow': follow, 'following_count': following_count, 'follower_count': follower_count})
...
def follow_list(request, username):
    user = User.objects.prefetch_related("follower", "following").get(username=username, is_active=True)
    return render(request, 'social/follow.html', {'user': user})
